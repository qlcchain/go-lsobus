...
func setupOrderParams() *api.OrderParams {
	createParams := &api.OrderParams{
		Buyer:  &api.PartnerParams{ID: "CBC", Name: "CBC"},
		Seller: &api.PartnerParams{ID: "PCCW", Name: "PCCW"},
	}

	uniItem := &api.UNIItemParams{
		SiteID:    "site111",
		PortSpeed: 1000,
	}
	...
	uniItem.BillingParams = &api.BillingParams{
	...
	lineItem.BillingParams = &api.BillingParams{
		StartTime: time.Now().Unix(),
		EndTime:   time.Now().Add(24 * time.Hour).Unix(),
		Currency:  "USD",
		Price:     56.78,
